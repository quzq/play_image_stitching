    ...
    def registration(self,img1,img2):
        ...
        for m1, m2 in raw_matches:
            if m1.distance < self.ratio * m2.distance:
                img1_pt = [list(map(int, kp1[m.queryIdx].pt)) for m in matches] # image1の座標を取得 
                img2_pt = [list(map(int, kp2[m.trainIdx].pt)) for m in matches] # image2の座標を取得
                if img1_pt.y < img2_pt.y:
